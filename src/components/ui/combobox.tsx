...
function ComboboxList({
  ...
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        'max-h-72 touch-pan-y scroll-py-1 overflow-y-auto overscroll-contain p-1 [-webkit-overflow-scrolling:touch] data-empty:p-0',
        className,
      )}
      onTouchStart={(event) => {
      ...
      onTouchMove={(event) => {
        ...
        if (nextY != null && previousY != null) {
          containScrollBoundary(event, previousY - nextY);
        }

        event.stopPropagation();
        onTouchMove?.(event);
      }}
      onWheel={(event) => containWheelEvent(event, onWheel)}
      {...props}
    />
  );
}

function containWheelEvent(
  event: React.WheelEvent<HTMLDivElement>,
  onWheel?: React.WheelEventHandler<HTMLDivElement>,
) {
  containScrollBoundary(event, event.deltaY);
  event.stopPropagation();
  onWheel?.(event);
}

function containScrollBoundary(
