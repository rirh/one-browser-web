...
const CHROMIUM_ASSET_NAME_PATTERN =
  /^one-browser-chromium-(macos|windows)-(arm64|x64)-(\d+\.\d+\.\d+(?:\.\d+)?)(?:-([0-9a-f]{64}))?\.tar\.gz$/i;
...
const archiveFileSchema = z.custom<File | null>().superRefine((file, ctx) => {
  ...
  if (!parseChromiumAssetFileName(file.name)) {
    ctx.addIssue({
      code: 'custom',
      message:
        '文件名需符合 one-browser-chromium-{platform}-{arch}-{version}[-{sha256}].tar.gz',
    });
  }
});
...
export const browserAssetUploadSchema = z.object({
  ...
  version: z
    ...
    .regex(
      ...
      '版本需为日期版或旧 Chromium 版，例如 26.716.2105',
    ),
  sha256: z
    .string()
    .trim()
...
export function parseChromiumAssetFileName(fileName: string) {
  ...
  return {
    ...
    version: match[3],
    sha256: match[4]?.toLowerCase() ?? null,
  };
}
